...
In `Cargo.toml`, add `pleco = "0.5"` to your dependencies (TODO: cargo.toml will be obvious to find/understand? Wasn't mentioned earlier). Then, at the top of `src/lib.rs`, add `use pleco::Board;`. Now, we have access to a chess board and can manipulate it with code such as this (TODO: the 'such as this' confuses me—is this not just the correct code? Or are you trying to say this is merely one way to implment the desired result (allowing us to manipulate the board?)):
...
The [pleco docs](https://github.com/pleco-rs/Pleco#using-pleco-as-a-library) show everything you can do using the pleco library. But this isn't very interesting! We want to play chess with other people. Let's start by creating a persisted state for the chess app and an IPC format for sending messages to other nodes.
...
(If you want to make a better (TODO: what does better mean here?) CLI app, consider parsing IPC as a string...)
...
serde_json = "1.0"
url = "*"
uqbar_process_lib = { git = "ssh://git@github.com/uqbar-dao/process_lib.git", rev = "a4cbe91" }
